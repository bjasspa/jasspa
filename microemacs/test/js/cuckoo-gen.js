...
if (argsNamed.Exists("xsl"))
    ...
    xslfile = WScript.CurrentDirectory + "\\cuckoo.xsl";
var bKey = null;
var progid = null;
    ...
    if (fso.FolderExists(xmldir)) {
        ...
        for (; !files.atEnd(); files.moveNext()) {
            ...
            if (n.substring(l) == "W.xml")
                process(xslfile, xmldir + "\\" + n, todir + "\\" + s + ".html", progid);
        }
    }
} else {
    if (argsNamed.Exists("file")) {
    	...
    	if (argsNamed.Exists("toFile"))
    	    ...
    	    if (argsNamed.Exists("toDir")) {
                ...
                process(xslfile, xmlname, toDir + f1.replace("W.xml", ".html"), progid);
    	    } else
                process(xslfile, xmlname, xmlname.replace("W.xml", ".html"), progid);
        }
    } else
        WScript.Echo(
