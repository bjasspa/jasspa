<? // following is accepted layout for space based 4 char indent
...
function details_page($id){
    ...
    $db->query("select tt.todo_id from todo_todos tt, todo_project_members tpm ".
               "where tt.todo_id=$id and tt.project_id=tpm.project_Id and tpm.member_id=".$HTTP_SESSION_VARS['usr']->getUserid());
    ...
    if (!$db->query("SELECT t.*,p.project_name FROM todo_todos t, todo_projects p WHERE todo_id=$id AND t.project_id=p.id"))
    return false;
    ...
    $db->query("SELECT n.note_id,n.text,n.date,u.first_name,u.last_name FROM todo_notes n, todo_users u".
               "WHERE todo_id=$id AND n.usernr=u.usernr ORDER BY n.date");
    $anzahl = $db->num_rows();
