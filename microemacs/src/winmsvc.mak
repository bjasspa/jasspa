...
CCDEFS   = /D_WIN32 /D_ARCHITEC=$(ARCHITEC) /D_TOOLKIT=$(TOOLKIT) /D_TOOLKIT_VER=$(TOOLKIT_VER) /D_PLATFORM_VER=$(PLATFORM_VER) /D_$(BIT_SIZE)BIT /D_WIN$(BIT_SIZE) /D_WIN32_WINNT=0x0602 /DWINVER=0x0602 /W3 /Zi /D_CRT_SECURE_NO_DEPRECATE /D_CRT_NONSTDC_NO_DEPRECATE /I$(TRDPARTY)\tfs /DmeVER_CN=$(meVER_CN) /DmeVER_YR=$(meVER_YR) /DmeVER_MN=$(meVER_MN) /DmeVER_DY=$(meVER_DY)
...
$(TRDPARTY)\tfs\$(BOUTDIR)\tfs.lib:
	cd $(TRDPARTY)\tfs
	$(MK) -f $(MAKEFILE).mak BCFG=$(BCFG) LSTT=$(LSTT) ARCHITEC=$(ARCHITEC) BIT_SIZE=$(BIT_SIZE)
	cd $(MAKEDIR)

clean:
	if exist $(OUTDIR)\ $(RMDIR) $(OUTDIR)
	cd $(TRDPARTY)\tfs
	$(MK) -f $(MAKEFILE).mak clean BCFG=$(BCFG) LSTT=$(LSTT) ARCHITEC=$(ARCHITEC) BIT_SIZE=$(BIT_SIZE)
	cd $(MAKEDIR)
...
spotless: clean
	...
	$(RM) tags
	cd $(TRDPARTY)\tfs
	$(MK) -f $(MAKEFILE).mak spotless BCFG=$(BCFG) LSTT=$(LSTT) ARCHITEC=$(ARCHITEC) BIT_SIZE=$(BIT_SIZE)
	cd $(MAKEDIR)
