...
#define _SEARCH_PATH _DEFAULT_SEARCH_PATH
    ...
    if(searchPath != NULL)
    {
        ...
        /* we just need to add the $user-path to the front if not there already */
        if((meUserPath != NULL) &&
...
meFrameXTermDrawSpecialChar(meFrame *frame, int x, int y, meUByte cc)
{
    ...
    case 0x08:          /* Visible space - mid height dot '.' */
        if(mecm.fdepth > 16)
        {
            ...
            XDrawLine(mecm.xdisplay, meFrameGetXWindow(frame), meFrameGetXGC(frame), x+mecm.fhwidth, y+mecm.fhdepth-1, x+mecm.fhwidth, y+mecm.fhdepth-1);
        break;
 ...
 * Checks $TERM_PROGRAM and $TERM environment variables for known terminals.
 * Returns 1 if SGR mouse is supported, 0 otherwise.
 */
static int
TTtermHasSGRMouse(void)
{
    ...
    termProgram = meGetenv("TERM_PROGRAM");
    if(termProgram != NULL)
    {
...
#endif
        {
            /* Fallback: use hardcoded SGR mouse sequences when XM termcap is not available */
            if((nCfg & meMOUSE_ENBLE) && TTtermHasSGRMouse())
            {
                if(cCfg & meMOUSE_ENBLE)
                    fputs("\033[?1006h\033[?1000h", stdout);
            }
