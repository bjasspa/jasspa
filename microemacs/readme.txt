JASSPA MicroEmacs - INTRODUCTION

MicroEmacs is an Emacs like text editor for developers, its great strength is its speed, fast key-bindings and powerful
macro language. If you are looking for a WYSIWYG, menu driven editor this is probably not for you!

MicroEmacs also has a couple of other base limitations, firstly, no real Unicode support - so you you are constantly
...
JASSPA MicroEmacs - VERY QUICK START GUIDE

For your platform download the single-file binary zip file, extract the executable and run.
...
For a more complete experience execute command init-session, this creates a small user configuration directory in your
user area, allowing MicroEmacs to save setup changes and download supporting files such as the comprehensive help docs
and spelling dictionaries for numerous languages.


JASSPA MicroEmacs - SLOWER QUICK START GUIDE
...
2. Download the following zip package files:
     ...
     - Binaries for your platform (Jasspa_MicroEmacs_<VERSION>_<PLATFORM>_binaries.zip)
     - Macro files (Jasspa_MicroEmacs_<VERSION>_macros.zip)
     - Help file (Jasspa_MicroEmacs_<VERSION>_hlpehf.zip)
     - Spelling dictionaries for any language you require (Jasspa_MicroEmacs_<VERSION>_ls<LANGUAGE>.zip)

3. Extract downloaded zip files into the ~/MicroEmacs directory you created in (1), this should have created:

     ~/MicroEmacs/bin directory containing platform directories for each of the binary packages you downloaded and
       extracted.

     ~/MicroEmacs/macros directory containing many macro files (*.emf), the me.ehf help file and other supporting
       files.

     ~/MicroEmacs/spelling directory containing the dictionaries of the languages you downloaded.
...
6. Try running:
   ...
   This should always work on Windows but may not on Unix systems, particularly macOS, where an X server is required
   to support the window interface. Fixing issues causing mew to not run is beyond the scope of this document.
