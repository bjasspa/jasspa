 ...
 *  Object Name   : $RCSfile: uappend.c,v $
...
tfs_append (int options, char *appendfile, char *outputfile, char *fileordir)
{
    ...
    if (outputfile != NULL)
    {
        if ((status = tfs_filecopy (options, appendfile, outputfile)) != 0)
            return status;
    ...
    if ((status = stat (fileordir, &stat_buf)) == 0)
    {
        ...
        if ((stat_buf.st_mode & S_IFDIR) != 0)
        {
            ...
            {
                /* Build the archive */
                status = tfs_build (options, fp, fileordir);
                fclose (fp);
            }
        }
        else if ((stat_buf.st_mode & S_IFREG) != 0)
            status = tfs_fileappend (options, fileordir, appendfile);
        else
        {
