...
sequence color_spec
integer color_code, prior_code

color_spec = {0,0,0}

global function read_24bit_bitmap(sequence file_name)
-- read a 24-bit bitmap (.BMP) file into a 2-d sequence of sequences (image),
-- a reduced & coded palette, and the number of matches of each color.
-- return {palette,image,matches}; NOTE: palette size may be anywhere from 1
-- to (in theory) 4 million. Also note that palette values are divided by 4
-- already, unlike the standard read_bitmap(); they are also converted into
-- integers by multiplying the R component by 4096, the G by 64, and the B
-- by 1 and adding them. Finally, a third value is also returned (although
-- it can be safely ignored.)

    atom Size 
    integer Type, Xhot, Yhot, Planes, BitCount, p, indx, rowbytes,
            row_start_indx
    atom Width, Height, Compression, OffBits, SizeHeader, 
	 SizeImage, XPelsPerMeter, YPelsPerMeter, ClrUsed,
	 ClrImportant, NumColors
    sequence Palette, two_d_bits, bit_row, matches, swap, Bits
    ...
    else -- SizeHeader = OLDHDRSIZE or something else
	close(fn)
	return BMP_UNSUPPORTED_FORMAT
    end if
    ...
    rowbytes = row_bytes (BitCount, Width)
    Bits = get_c_block(rowbytes * Height)
    --Bits = floor (Bits / 4)
    for i = 1 to length (Bits) do
        color_code = Bits[i]
        Bits[i] = floor (color_code / 4)
    end for

    two_d_bits = repeat (repeat (0, Width), Height)
    p = 0
    row_start_indx = 1
    for h = 1 to Height do
       bit_row = two_d_bits[h]
       indx = row_start_indx
       puts (1, '.')
       for w = 1 to Width do
          color_spec = Bits[indx..indx+2]
          color_code = color_spec[1]*4096 + color_spec[2]*64 + color_spec[3]
          if (color_code = prior_code) then
	     bit_row[w] = p - 1
	     matches[p] = matches[p] + 1
	  else
             p = find (color_code, Palette)
	     if (p > 0) then
		matches[p] = matches[p] + 1
	     else
                Palette = Palette & color_code
		matches = matches & 1
		p = length (Palette)
	     end if
             bit_row[w] = p - 1
             prior_code = color_code
	  end if
          indx = indx + 3
       end for
       two_d_bits[h] = bit_row
       row_start_indx = row_start_indx + rowbytes
    end for
    puts (1, '\n')

    close(fn)

     -- flip the image now (its stored upside down)
    for i = 1 to floor (Height / 2) do
        swap = two_d_bits[i]
        two_d_bits[i] = two_d_bits[Height-i+1]
        two_d_bits[Height-i+1] = swap
    end for
	...
	return error_code 
     ...
     -- colors appear to be stored in BGR order, so reverse them
    for i = 1 to length (Palette) do
       color_code = Palette[i]
       color_spec[1] = remainder (color_code, 64)
       color_code = floor (color_code / 64)
       color_spec[2] = remainder (color_code, 64)
       color_spec[3] = floor (color_code / 64)
       Palette[i] = color_spec
    end for

    return {Palette, two_d_bits, matches}
end function
...
with 472333662 -- delete this statement if it causes an error
