 ...
 * form-php.js
 * Convenience tool to generate a PHP processing the request of a Cuckoo document
 * Version 0.1.1
 ...
 * Usage:
 * form-php.js /dir:source-directory|/file:file [/toDir:target-directory|/toFile:target-file] [/xsl:xsl-file]
 * Example:
 * form-php.js /dir:D:\cuckoo /toDir:D:\cuckoo /xsl:D:\cuckoo\cuckoo.xsl
 * form-php.js /file:D:\cuckoo\myfile-phpF.xml /toFile:D:\cuckoo\myfile.php /xsl:D:\cuckoo\form-php.xsl
 */
var xslfile = null;
var todir = null;
...
if (argsNamed.Exists("xsl"))
    ...
    xslfile = WScript.CurrentDirectory + "\\form-php.xsl";
var bKey = null;
var progid = null;
...
if (argsNamed.Exists("dir")) {
    ...
    if (fso.FolderExists(xmldir)) {
        ...
        for (; !files.atEnd(); files.moveNext()) {
            ...
            var l = n.length - "-phpF.xml".length;
            var s = n.substring(0, l);
            if (n.substring(l) == "-phpF.xml")
                process(xslfile, xmldir + "\\" + n, todir + "\\" + s + ".php", progid);
        }
    }
} else {
    if (argsNamed.Exists("file")) {
    	...
    	if (argsNamed.Exists("toFile"))
    	    ...
    	    if (argsNamed.Exists("toDir")) {
                ...
                process(xslfile, xmlname, toDir + f1.replace("-phpF.xml", ".php"), progid);
    	    } else
                process(xslfile, xmlname, xmlname.replace("-phpF.xml", ".php"), progid);
        }
    } else
        WScript.Echo(
            "Usage: form-php.js /dir:source-directory|/file:file [/toDir:target-directory|"
